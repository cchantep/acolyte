...
final class ResponseMakerSpec
  ...
  "Query response maker" should {
    ...
    "be working for Iterable[BSONDocument]" in {
      val makr = implicitly[QueryResponseMaker[Iterable[BSONDocument]]]
      val cid = channelId()

      makr(cid, documents) aka "response" must beSome[Try[Response]].which {
        prepared =>
    ...
    "be working for an error message (String)" in {
      ...
      makr(cid, "Custom error") aka "response" must beSome[
        Try[Response]
      ].which { prepared =>
  ...
  "Write response maker" should {
    ...
    "be a successful one for boolean (updatedExisting)" in {
      ...
      makr(cid, 1 -> true) aka "response" must beSome[Try[Response]].which {
        prepared =>
    ...
    "be a successful one for Unit (updatedExisting = false)" in {
      ...
      makr(cid, ()) aka "response" must beSome[Try[Response]].which {
        prepared =>
    ...
    "be working for an error with code (String, Int)" in {
      ...
      makr(cid, "Custom error #2" -> 7) aka "response" must beSome[Try[
        Response
      ]].which { prepared =>
