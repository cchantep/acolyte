  ...
  "Reponse" should {
    "be made for error message" >> {
      "using generic factory" in {
        QueryResponse("error message #1") aka "prepared" must beLike {
          case prepared =>
            prepared(channelId()) aka "applied" must beSome[
              Try[Response]
            ].which {
    ...
    "be made for successful result" >> {
        ...
        QueryResponse(Doc1) aka "prepared" must beLike {
          case prepared =>
            prepared(channelId()) aka "applied" must beSome[
              ...
              _ aka "query response" must beResponseLike {
      ...
      "with a many documents using named factory" in {
        QueryResponse
          .successful(Doc1, BSONDocument("b" -> 3))
          .aka("prepared") must beLike {
          case prepared =>
            prepared(channelId()) aka "applied" must beSome[Try[Response]]
              .which(_ aka "response" must beResponseLike {
      ...
      "for count command" in {
        QueryResponse.count(3) aka "prepared" must beLike {
          case prepared =>
            prepared(channelId()) aka "applied" must beSome[Try[Response]]
              .which(_ aka "reponse" must beResponseLike {
                case ValueDocument(
                      ("ok", BSONInteger(1)) ::
                      ("n", BSONInteger(3)) :: Nil
                    ) :: Nil =>
