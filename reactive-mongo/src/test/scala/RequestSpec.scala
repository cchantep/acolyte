...
final class RequestSpec
  ...
  "Collection name" should {
    "match" >> {
      "request #1" in {
        request1 aka "request" must beLike { case Request("db1.col1", _) => ok }
      }

      "request #2" in {
        request2 aka "request" must beLike { case Request("db1.col2", _) => ok }
      }
    }

    "not match" >> {
      "request #1" in {
        request1 aka "request" must not(beLike {
          case Request("db1.col2", _) => ok
        })
      }

      "request #2" in {
        request2 aka "request" must not(beLike {
  ...
  "Ordered property list" should {
    ...
    "be extracted as BSON values for exactly 2 properties" in {
      request1 aka "request #1" must beLike {
        case Request(col, SimpleBody((k1, v1) :: (k2, v2) :: Nil)) =>
          col aka "collection" must_=== "db1.col1" and (k1 aka "key #1" must_=== "email") and (v1 aka "value #1" must_=== BSONString(
            "em@il.net"
          )) and (k2 aka "key #2" must_=== "age") and (v2 aka "value #2" must_=== BSONInteger(
            11
          ))
      }
    }
    ...
    "be extracted as Scala values for exactly 2 properties" in {
      request1 aka "request #1" must beLike {
        case Request(
              col,
              SimpleBody((k1, BSONString(v1)) :: (k2, BSONInteger(v2)) :: Nil)
            ) =>
          col aka "collection" must_=== "db1.col1" and (k1 aka "key #1" must_=== "email") and (v1 aka "value #1" must_=== "em@il.net") and (k2 aka "key #2" must_=== "age") and (v2 aka "value #2" must_=== 11)
      }
    }
    ...
    "be extracted as BSON values from nested document" in {
      request2 aka "request #1" must beLike {
        case Request(
          ...
          email aka "email" must_=== BSONString(
            "em@il.net"
          ) and (meta aka "meta" must_=== BSONString(
            "y"
          )) and (gt aka "gt" must_=== BSONInteger(
            10
          )) and (prio aka "priority" must_=== BSONDouble(0.25D))
      }
    }
    ...
    "be extracted as Scala values from nested document" in {
      request2 aka "request #1" must beLike {
        case Request(
          ...
          email aka "email" must_=== "em@il.net" and (meta aka "meta" must_=== "y") and (gt aka "gt" must_=== 10) and (prio aka "priority" must_=== 0.25D)
      }
    }
  }
  ...
  "Unordered property list" should {
    ...
    "match 'email' property" >> {
      "on request #1" in {
        request1 aka "request" must beLike {
          case Request(
                "db1.col1",
                SimpleBody(Email(BSONString("em@il.net")))
              ) =>
            ok
        }
      }
    ...
    "extract 'email' property as BSON value" >> {
      "on request #1" in {
        request1 aka "request" must beLike {
          case Request("db1.col1", SimpleBody(Email(email))) =>
    ...
    "extract 'email' property as Scala value" >> {
      "on request #1" in {
        request1 aka "request" must beLike {
          case Request("db1.col1", SimpleBody(Email(BSONString(email)))) =>
      ...
      "on request #2 in same order" in {
        request2 aka "request #2" must beLike {
          case Request(
                ...
                SimpleBody(
                  Email(BSONString("em@il.net")) &
                  Age(ValueDocument(Greater(BSONInteger(10))))
                )
              ) =>
            ok
    ...
    "extract 'age' properties as BSON values" >> {
      "on request #2 in same order" in {
        request2 aka "request" must beLike {
          case Request(
                "db1.col2",
                SimpleBody(Age(ValueDocument(Meta(meta) & Greater(gt))))
              ) =>
            gt aka "gt" must_=== BSONInteger(
              10
            ) and (meta aka "meta" must_=== BSONString("y"))
        }
      }
      ...
      "on request #2 in reverse order" in {
        request2 aka "request" must beLike {
          case Request(
                "db1.col2",
                SimpleBody(Age(ValueDocument(Greater(gt) & Meta(meta))))
              ) =>
            gt aka "gt" must_=== BSONInteger(
              10
            ) and (meta aka "meta" must_=== BSONString("y"))
        ...
        case AggregateRequest(
              ...
              List(
                ValueDocument(
                  (
                    "$match",
                    ValueDocument(("category", BSONString("A")) :: Nil)
                  ) :: Nil
                )
              ),
              (
                "cursor",
                ValueDocument(("batchSize", BSONInteger(123)) :: Nil)
              ) :: Nil
            ) =>
          ok
      }
    ...
    "be extracted from an $in clause" in {
      BSONDocument("selector" -> BSONDocument("$in" -> BSONArray("A", "B")))
        ...
        case ValueDocument(
              (
                "selector",
                InClause(BSONString("A") :: BSONString("B") :: Nil)
              ) :: Nil
            ) =>
          ok
      }
    ...
    "be extracted from an $nin clause" in {
      BSONDocument("selector" -> BSONDocument("$nin" -> BSONArray("A", "B")))
        ...
        case ValueDocument(
              (
                "selector",
                NotInClause(BSONString("A") :: BSONString("B") :: Nil)
              ) :: Nil
            ) =>
          ok
      }
