...
class ConnectionHandlerSpec
      ...
      ConnectionHandler.empty aka "connection handler" must beLike {
        case h =>
          h.queryHandler(channelId(), query1)
            .aka("query result") must beNone and (h
  ...
  "Handler with query handler" should {
    "return some query result" in {
      val h = implicitly[QueryHandler] { (_: Request) =>
        QueryResponse(BSONDocument("a" -> "b"))
      }
      val handler = ConnectionHandler.empty

      ConnectionHandler(h)
        .queryHandler(channelId(), query1)
        .aka("query result #1") must beSome and (handler.queryHandler(
        channelId(),
        query1
      ) aka "query result #2" must beNone).and(
        handler
          .withQueryHandler(h)
          .queryHandler(channelId(), query1)
  ...
  "Handler with write handler" should {
    "return some write error" in {
      val h = implicitly[WriteHandler] { (_: WriteOp, _: Request) =>
        WriteResponse("Error #1" -> 8)
      }
      val handler = ConnectionHandler()

      ConnectionHandler(writeHandler = h)
        .writeHandler(channelId(), write1._1, write1._2)
        .aka("write result #1") must beSome and (handler
        .writeHandler(channelId(), write1._1, write1._2)
        .aka("write result #2") must beNone) and (handler
        .withWriteHandler(h)
        .writeHandler(channelId(), write1._1, write1._2)
    ...
    "be combined using orElse" in {
      val handler: ConnectionHandler = ConnectionHandler({ (_: Request) =>
        QueryResponse(BSONDocument("a" -> "b"))
      })
      ...
      handler.queryHandler(channelId(), query1) aka "query result" must beSome[
        Try[Response]
      ].which {
        _ aka "response" must beResponseLike {
          case ValueDocument(("a", BSONString("b")) :: Nil) :: Nil => ok
        }
      }
    }
  ...
  "Complete handler" should {
    "return expected query result #1" in {
      chandler1.queryHandler(
        channelId(),
        query1
      ) aka "query result" must beSome[Try[Response]].which {
        _ aka "response" must beResponseLike {
    ...
    "return expected write result #1" in {
      chandler1
        .writeHandler(channelId(), write1._1, write1._2)
        .aka("write result") must beSome[Try[Response]]
        .which(_ aka "response" must beWriteError("Error #2"))
    }

    "return expected write result #2" in {
      chandler1
        .writeHandler(channelId(), write2._1, write2._2)
        .aka("write result") must beSome[Try[Response]]
        .which(_ aka "response" must beResponseLike {
          case ValueDocument(
                ("ok", BSONInteger(1)) ::
    ...
    "return no write result" in {
      chandler1
        .writeHandler(channelId(), write3._1, write3._2)
        .aka("write result") must beNone
    }
  }
}
...
trait ConnectionHandlerFixtures {
  ...
  lazy val chandler1 = ConnectionHandler(
    QueryHandler {
      case Request(col, _) if col.endsWith("test1") =>
        QueryResponse(BSONDocument("b" -> 3))

      case Request(col, _) if col.endsWith("test2") =>
        QueryResponse(
