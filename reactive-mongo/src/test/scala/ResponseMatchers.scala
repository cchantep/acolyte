      ...
      def apply[R <: Try[Response]](e: Expectable[R]) = {
        e.value aka "prepared" must beSuccessfulTry.which { resp =>
          ...
          result(
            ...
            s"response is valid but ${r.message}",
            e
          )
        }
      }
    ...
    new Matcher[Try[Response]] {
      ...
      def apply[R <: Try[Response]](e: Expectable[R]) = {
        e.value aka "prepared" must beSuccessfulTry.which { resp =>
          val r = f
            .applyOrElse(
              parseResponse(resp).toList,
              (_: List[BSONDocument]) => ko
            )
            .toResult
    ...
    new Matcher[Try[Response]] {
      ...
      def apply[R <: Try[Response]](e: Expectable[R]) =
        e.value aka "prepared" must beSuccessfulTry.which { r =>
          r.reply.inError aka "in-error" must beTrue and (parseResponse(
            r
          ).toList aka "response" must beLike {
        ...
        e.value aka "prepared" must beSuccessfulTry.which {
          parseResponse(_).toList aka "response" must beLike {
            case ValueDocument(
              ...
              err aka "error message (err)" must_=== msg and (errmsg aka "error message (errmsg)" must_=== msg) and (c aka "error code" must_=== code
                .getOrElse(-1))
          }
        }
    }
