...
object RowListSpec extends Specification with RowListTest {
  ...
  "Factory" should {
    ...
    "create resultset using projection" >> {
      lazy val list = RowLists.rowList2(meta._1, meta._2.withNullable(true)).
        append("Foo", 1).append("Bar", 2)
  ...
  "Column classes" should {
    ...
    "be String, Double, Date" in {
      ...
      RowLists.rowList3(
        classOf[String],
        classOf[Double],
        classOf[java.util.Date]).getColumnClasses must_=== cs
  ...
  "Result set statement" should {
    "be null" in {
      (new RowList1.Impl(classOf[String]).resultSet.
        ...
        and(new RowList1.Impl(classOf[String]).resultSet.withStatement(null).
          getStatement aka "statement" must beNull)
    ...
    "be expected one" in {
      ...
      new RowList1.Impl(classOf[String]).resultSet.withStatement(s).
  ...
  "Result set fetch size" should {
    "be 1" in {
      (new RowList1.Impl(classOf[String]).append("str").
    ...
    "be 2" in {
      (new RowList1.Impl(classOf[String]).append("a").append("b").
        resultSet.getFetchSize aka "size" must_=== 2)
  ...
  "Max rows limit" should {
    ...
    "drop 2 rows at end (max = 8)" in {
      rows.resultSet(8) aka "resultset" must_=== (
        RowLists.intList(10, 9, 8, 7, 6, 5, 4, 3).resultSet)
    }

    "extract only 3 rows" in {
      rows.resultSet(3) aka "resultset" must_=== (
        RowLists.intList(10, 9, 8).resultSet)
    }

    "get all rows" in {
      rows.resultSet(11) aka "resultset" must_=== rows.resultSet and (
        rows.resultSet(0) must_=== rows.resultSet) and (
          rows.resultSet() must_=== rows.resultSet)
    }
  }
  ...
  "Object column by index" should {
    "not be read when not on a row" in {
      ...
      lazy val rs = new RowList1.Impl(classOf[String]).
        withLabel(1, "n").append("str").resultSet
    ...
    "be null" in {
      lazy val rs = new RowList1.Impl(
        classOf[java.lang.Float]).withLabel(1, "n").
        append(null.asInstanceOf[java.lang.Float]).resultSet
    ...
    "not convert without type" in {
      lazy val rs = new RowList1.Impl(classOf[String]).withLabel(1, "n").
    ...
    "not convert incompatible type" in {
      ...
      (rs.getObject(1, classOf[Date]).
        aka("byIndex") must throwA[SQLException]("Incompatible type")).
        and(rs.getObject("n", classOf[Date]).
    ...
    "convert compatible type" in {
      val d = new Date(System.currentTimeMillis())
      lazy val rs = new RowList1.Impl(classOf[Date]).withLabel(1, "n").
        append(d).resultSet
      rs.next

      (rs.getObject(1, classOf[java.util.Date]) aka "byIndex" must_=== d).
    ...
    "convert compatible temporal type" in {
      ...
      (rs.getObject(1, classOf[Time]) aka "byIndex" must_=== t).
        and(rs.getObject("n", classOf[Timestamp]).
          aka("byLabel") must_=== ts)

    }

    "convert compatible numeric type" in {
      lazy val rs = new RowList1.Impl(classOf[Int]).withLabel(1, "n").
        append(1).resultSet
      rs.next

      (rs.getObject(1, classOf[java.lang.Float]).
        aka("byIndex") must_=== 1F).
        and(rs.getObject("n", classOf[java.lang.Double]).
          aka("byLabel") must_=== 1D)
  ...
  "Object column by name" should {
    "not be read when not on a row" in {
      new RowList1.Impl(classOf[String]).withLabel(1, "n").
        append("str").resultSet.
    ...
    "be null" in {
      lazy val rs = new RowList1.Impl(
        classOf[java.lang.Float]).withLabel(1, "name").
        append(null.asInstanceOf[java.lang.Float]).resultSet
  ...
  "String column from result set" should {
    "not be read by index when not on a row" in {
      (new RowList1.Impl(classOf[String]).append("str").resultSet.
        getString(1) aka "getString" must throwA[SQLException](
          message = "Not on a row")).
        and(new RowList1.Impl(classOf[String]).withLabel(1, "n").
          append("str").resultSet.getString("n").
    ...
    "be null" in {
      val rs = (new RowList1.Impl(classOf[String]).
        withLabel(1, "n").append(null.asInstanceOf[String])).resultSet
  ...
  "Boolean column from result set" should {
    "not be read by index when not on a row" in {
      (new RowList1.Impl(classOf[Boolean]).append(true).resultSet.
        getBoolean(1) aka "getBoolean" must throwA[SQLException](
          message = "Not on a row")).
        and(new RowList1.Impl(classOf[Boolean]).withLabel(1, "n").
          append(false).resultSet.getBoolean("n").
          aka("getBoolean") must throwA[SQLException]("Not on a row"))

    }

    "be expected one" in {
      val rs = new RowList1.Impl(classOf[Boolean]).withLabel(1, "n").
        append(true).resultSet
      rs.next
    ...
    "be null (false)" in {
      val rs = new RowList1.Impl(classOf[Boolean]).withLabel(1, "n").
        append(null.asInstanceOf[Boolean]).resultSet
    ...
    "converted to true by index" >> {
      val rs = Seq[(String, ResultSet)](
        ...
        "short" -> (new RowList1.Impl(classOf[Short]).append(3.toShort)).resultSet,
    ...
    "converted to false by label" >> {
      val rs = Seq[(String, ResultSet)](
        "char" -> (new RowList1.Impl(classOf[Char]).
          withLabel(1, "n").append('0')).resultSet,
        "byte" -> (new RowList1.Impl(classOf[Byte]).
          withLabel(1, "n").append(0.toByte)).resultSet,
        "short" -> (new RowList1.Impl(classOf[Short]).
          withLabel(1, "n").append(0.toShort)).resultSet,
        "int" -> (new RowList1.Impl(classOf[Int]).
          withLabel(1, "n").append(0)).resultSet,
        "long" -> (new RowList1.Impl(classOf[Long]).
          withLabel(1, "n").append(0L)).resultSet)
    ...
    "converted to true by label" >> {
      val rs = Seq[(String, ResultSet)](
        "char" -> (new RowList1.Impl(classOf[Char]).
          withLabel(1, "n").append('1')).resultSet,
        "byte" -> (new RowList1.Impl(classOf[Byte]).
          withLabel(1, "n").append(2.toByte)).resultSet,
        "short" -> (new RowList1.Impl(classOf[Short]).
          withLabel(1, "n").append(3.toShort)).resultSet,
        "int" -> (new RowList1.Impl(classOf[Int]).
          withLabel(1, "n").append(4)).resultSet,
        "long" -> (new RowList1.Impl(classOf[Long]).
          withLabel(1, "n").append(5L)).resultSet)
  ...
  "BigDecimal column from result set" should {
    ...
    "not be read by index when not on a row" in {
      (new RowList1.Impl(classOf[JBigDec]).append(v).resultSet.
        getBigDecimal(1) aka "get" must throwA[SQLException](
          message = "Not on a row")).
        and(new RowList1.Impl(classOf[JBigDec]).withLabel(1, "n").
          append(v).resultSet.getBigDecimal("n").
          aka("get") must throwA[SQLException]("Not on a row"))

    }

    "be expected one" in {
      val rs = new RowList1.Impl(classOf[JBigDec]).withLabel(1, "n").
        append(v).resultSet
      rs.next
    ...
    "be scaled one" in {
      val rs = new RowList1.Impl(classOf[JBigDec]).withLabel(1, "n").
        append(new JBigDec("1.2345")).resultSet

      rs.next

      (rs.getBigDecimal(1, 2).
        aka("big decimal by index") must_=== new JBigDec("1.23")).
    ...
    "be null" in {
      val rs = new RowList1.Impl(classOf[JBigDec]).withLabel(1, "n").
        append(null.asInstanceOf[JBigDec]).resultSet
      rs.next
    ...
    "converted by index" >> {
      val rs = Seq[(String, ResultSet)](
        "byte" -> (new RowList1.Impl(classOf[Byte]).
          append(v.intValue.toByte)).resultSet,
        "short" -> (new RowList1.Impl(classOf[Short]).
          append(v.intValue.toShort)).resultSet,
        "int" -> (new RowList1.Impl(classOf[Int]).
          append(v.intValue)).resultSet,
        "long" -> (new RowList1.Impl(classOf[Long]).
          append(v.longValue)).resultSet,
        "float" -> (new RowList1.Impl(classOf[Float]).
          append(v.floatValue)).resultSet,
        "double" -> (new RowList1.Impl(classOf[Double]).
          append(v.doubleValue)).resultSet)
    ...
    "converted by label" >> {
      val rs = Seq[(String, ResultSet)](
        "byte" -> (new RowList1.Impl(classOf[Byte]).withLabel(1, "n").
          append(v.intValue.toByte)).resultSet,
        "short" -> (new RowList1.Impl(classOf[Short]).withLabel(1, "n").
          append(v.intValue.toShort)).resultSet,
        "int" -> (new RowList1.Impl(classOf[Int]).withLabel(1, "n").
          append(v.intValue)).resultSet,
        "long" -> (new RowList1.Impl(classOf[Long]).withLabel(1, "n").
          append(v.longValue)).resultSet,
        "float" -> (new RowList1.Impl(classOf[Float]).withLabel(1, "n").
          append(v.floatValue)).resultSet,
        "double" -> (new RowList1.Impl(classOf[Double]).withLabel(1, "n").
          append(v.doubleValue)).resultSet)
  ...
  "Array column from result set" should {
    ...
    "be expected one" in {
      val rs = new RowList1.Impl(classOf[ImmutableArray[String]]).
        withLabel(1, "n").append(v).resultSet
      rs.next
    ...
    "converted by label" >> {
      val rs = Seq[(String, ResultSet)](
        "raw array" -> (new RowList1.Impl(classOf[Array[String]])).
          withLabel(1, "n").append(Array("a", "b", "c")).resultSet)
    ...
    s"$name column from result set" should {
      "not be read by index when not on a row" in {
        (byIndex(new RowList1.Impl(mf.runtimeClass.asInstanceOf[Class[D]]).
          append(v).resultSet, 1).
          aka("get") must throwA[SQLException](
            message = "Not on a row")).
          and(byLabel(new RowList1.Impl(mf.runtimeClass.asInstanceOf[Class[D]]).
            withLabel(1, "n").append(v).resultSet, "n").
            aka("get") must throwA[SQLException]("Not on a row"))
      ...
      "be null" in {
        val c = Calendar.getInstance
        val rs = new RowList1.Impl(mf.runtimeClass.asInstanceOf[Class[D]]).
          withLabel(1, "n").append(null.asInstanceOf[D]).resultSet
      ...
      "converted by index" >> {
        val rs = Seq[(String, ResultSet)](
          "date" -> new RowList1.Impl(classOf[Date]).
            append(new Date(v.getTime)).resultSet,
          "time" -> new RowList1.Impl(classOf[Time]).
            append(new Time(v.getTime)).resultSet,
          "ts" -> new RowList1.Impl(classOf[Timestamp]).
            append(new Timestamp(v.getTime)).resultSet)
      ...
      "converted by label" >> {
        val rs = Seq[(String, ResultSet)](
          "date" -> new RowList1.Impl(classOf[Date]).withLabel(1, "n").
            append(new Date(v.getTime)).resultSet,
          "time" -> new RowList1.Impl(classOf[Time]).withLabel(1, "n").
            append(new Time(v.getTime)).resultSet,
          "ts" -> new RowList1.Impl(classOf[Timestamp]).withLabel(1, "n").
            append(new Timestamp(v.getTime)).resultSet)
    ...
    s"$name column from result set" should {
      "not be read by index when not on a row" in {
        (byIndex(new RowList1.Impl(mf.runtimeClass.asInstanceOf[Class[N]]).
          ...
          aka("get") must throwA[SQLException](
            message = "Not on a row")).
          and(byLabel(new RowList1.Impl(mf.runtimeClass.asInstanceOf[Class[N]]).
            withLabel(1, "n").append(v).resultSet, "n").
            aka("get") must throwA[SQLException]("Not on a row"))
      ...
      "converted by index" >> {
        val rs = Seq[(String, ResultSet)](
          "byte" -> new RowList1.Impl(classOf[Byte]).
            append(num.toInt(v).toByte).resultSet,
          "short" -> new RowList1.Impl(classOf[Short]).
            append(num.toInt(v).toShort).resultSet,
          "int" -> new RowList1.Impl(classOf[Int]).
            append(num.toInt(v)).resultSet,
          "long" -> new RowList1.Impl(classOf[Long]).
            append(num.toLong(v)).resultSet,
          "float" -> new RowList1.Impl(classOf[Float]).
            append(num.toFloat(v)).resultSet,
          "double" -> new RowList1.Impl(classOf[Double]).
            append(num.toDouble(v)).resultSet)
