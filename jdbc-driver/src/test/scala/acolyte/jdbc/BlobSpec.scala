...
object BlobSpec extends org.specs2.mutable.Specification {
  ...
  "Nil instance" should {
    ...
    "has empty binary stream" in {
      Blob.Nil.getBinaryStream.read aka "binary stream #1" must_=== -1 and (Blob.Nil
        .getBinaryStream(1, 2)
        .read aka "binary stream #2" must_=== -1)
    }
    ...
    "not find position" in {
      Blob.Nil.position(serialBlob, 0) aka "position" must_=== -1L and (Blob.Nil
        .position(serialBlob, 1) aka "position" must_=== -1L) and (Blob.Nil
        .position(testData, 0) aka "position" must_=== -1L) and (Blob.Nil
        .position(testData, 1) aka "position" must_=== -1L)
    }

    "truncate as no-op" in {
      (Blob.Nil.truncate(0) aka "truncate to 0" must not(throwA[SQLException]))
        .and(
          Blob.Nil.truncate(1) aka "truncate to 1" must not(
            throwA[SQLException]
          )
        )
    }
    ...
    "not be writable" in {
      (Blob.Nil.setBinaryStream(1) aka "write request @ 1" must (throwA[
        SQLFeatureNotSupportedException
      ]("Cannot write to empty BLOB"))).and(
        Blob.Nil.setBinaryStream(3) aka "write request @ 3" must (throwA[
          SQLException
        ]("Invalid position: 3"))
      )
    }
    ...
    "not be set missing data" in {
      Blob.Nil.setBytes(1, null) aka "missing data" must (throwA[
        IllegalArgumentException
      ]("No byte to be set"))
    }

    "throw SQL exception setting bytes at invalid position" in {
      Blob.Nil.setBytes(2, Array[Byte]()) aka "setting bytes" must (throwA[
        SQLException
      ]("Invalid position: 2"))
    }
    ...
    "throw exception setting bytes with invalid length" in {
      Blob.Nil.setBytes(
        ...
        0,
        -1
      ) aka "setting bytes" must (throwA[IllegalArgumentException](
